...
\item The real number k for which the equation, $2x^3+3x+4=0$ has two distinct real roots in $[0,1]$
...
\item If the equations $x^2+2x+3=0$ and $ax^2+bx+c=0$,a,b,c$\in$ $\mathbb{R}$ , have a   common   root , then $a:b:c$ is
	...
	\end{multicols}
	\end{enumerate}
\item If a $\in$ $\mathbb{R}$ and the equation  $-3(x-[x])^2 +2(x-[x])+a^2 =0$ (where [x] denotes the greatest integer $\leq$ x) has no integral solution, then all possible values of a lie in the interval: \hfill$\brak{JEE M 2014}$\\
          \begin{enumerate}[label=\alph*.]
	  \begin{multicols}{2}
	  \item $(-2,-1)$
